...
function CustomerFormSection({
    ...
    const fields = [
        { name: 'name', label: 'Họ và tên', type: 'text', placeholder: 'Nhập họ và tên', Icon: User },
        { name: 'email', label: 'Email', type: 'email', placeholder: 'Nhập email', Icon: Mail },
        { name: 'phone', label: 'Số điện thoại', type: 'tel', placeholder: 'Nhập số điện thoại', Icon: Phone },
    ];
                ...
                {fields.map(({ name, label, type, placeholder, Icon }) => (
                    ...
                    >
                        <div className="space-y-1.5 flex-1 min-w-0">
                            <p className="text-(--text-primary)/70 text-xs sm:text-sm">{label}</p>
                            <input
                                name={name}
                                type={type}
                                value={form[name]}
                        ...
                        <Icon color="var(--text-primary)" size={16} className="shrink-0 opacity-60" />
                    </div>
                ))}
            </div>
        </div>
