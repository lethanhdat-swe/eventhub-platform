...
function SearchSelect() {
  ...
  return (
    <div className="flex items-center gap-2 p-4 mx-8 rounded-2xl bg-(--background-color)/70 container">
      <div className="flex items-center flex-1 gap-3 px-4 py-2 rounded-xl bg-white/5">
      ...
      {action.map(({ icon: Icon, label, defaultValue }) => (
        <div key={label} className="flex items-center">
          <div className="w-px h-8 mr-4 bg-white/10" />
      ...
      {/* Search button */}
      <button className="px-6 py-3 ml-2 text-sm font-semibold text-white transition-all rounded-xl hover:opacity-90 hover:scale-105 bg-(--primary-color)">
        Search
      </button>
    </div>
  );
