...
function ContactInfo() {
  const contactInfo = [
    ...
    },
    {
      ...
      icon: Phone,
      title: 'Hotline',
      lines: ['1900 1234', '(8:00 - 22:00 mỗi ngày)'],
    },
    {
      id: 3,
      icon: Mail,
      title: 'Email hỗ trợ',
      lines: ['support@eventhub.vn'],
    },
    {
      id: 4,
      icon: Clock,
      title: 'Giờ làm việc',
      lines: ['Thứ 2 - Chủ nhật', '08:00 - 22:00 (kể cả ngày lễ)'],
    },
  ];
  ...
  return (
    <div className="py-1 px-6 border-l border-(--text-primary)/20">
        ...
        {contactInfo.map(({ id, icon: Icon, title, lines }) => (
          <div key={id} className="flex items-start gap-4 group">
            ...
            <div className="flex flex-col gap-1">
              <p className="text-(--text-primary) font-medium transition-colors duration-300 group-hover:text-(--primary-color)">
                {title}
              </p>
              {lines.map((line, i) => (
                <p
                  key={i}
                  className="text-sm text-(--text-primary)/70 transition-colors duration-300 group-hover:text-(--text-primary)/50"
                >
                  {line}
                </p>
              ))}
      ...
      <div className="flex flex-col gap-5 mt-15">
        <h1 className="text-(--text-primary) text-xl uppercase">
