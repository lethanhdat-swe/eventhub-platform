...
import {
  AlertCircle, AlertTriangle, ArrowLeft, Building2,
  CheckCircle2, Clock, Copy, CreditCard, QrCode,
} from 'lucide-react';
  ...
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-(--text-primary)/10 bg-(--surface-color)/45 p-3 sm:p-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={`mt-1 break-all font-semibold text-(--text-primary) text-sm sm:text-base ${valueClassName}`}>
          {value}
        </div>
      </div>
      {action}
...
function CopyButton({ value, copied, onCopy }) {
  return (
    ...
    >
      ...
      <span className="hidden xs:inline">{copied ? 'Đã copy' : 'Sao chép'}</span>
    </button>
  );
}
...
function PaymentQrPage() {
    ...
    const checkOrderStatus = async () => {
      ...
      try {
        ...
        if (status === 'PAID') {
          ...
          navigate(`/payment-success/${order.id}`, { replace: true, state: { order: latestOrder, sepay } });
          return;
        }
        if (status === 'CANCELLED') {
          hasTerminalStatusRef.current = true;
          navigate(`/payment-failed/${order.id}`, { replace: true, state: { order: latestOrder, sepay } });
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!hasPaymentInfo || !order?.id || !isExpired || hasExpiredHandledRef.current || hasTerminalStatusRef.current) return undefined;
    ...
    const handleExpiredPayment = async () => {
      try {
        ...
        navigate(`/payment-failed/${order.id}`, { replace: true, state: { order, sepay, reason: 'expired' } });
      }
    };
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-8">
          ...
          <section className="lg:col-span-7 rounded-3xl border border-(--text-primary)/10 bg-(--surface-color)/80 p-4 sm:p-5 shadow-[0_24px_80px_rgba(0,0,0,0.10)] lg:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className={`self-start inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs sm:text-sm font-semibold ${isExpired ? 'bg-red-500/10 text-red-500' : 'bg-amber-500/10 text-amber-500'}`}>
                <StatusIcon size={15} />
            ...
            <div className="mt-5 rounded-2xl border border-(--primary-color)/20 bg-(--primary-color)/10 p-3 sm:p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-(--text-primary)/60">Thời gian còn lại</p>
                ...
                <p className="text-xs sm:text-sm font-medium text-(--text-primary)/65">
                  {isExpired ? 'Phiên thanh toán đã hết thời gian.' : 'Vui lòng hoàn tất trước khi hết giờ.'}
                </p>
              </div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-(--text-primary)/10">
                <div className="h-full rounded-full bg-(--primary-color) transition-all duration-500" style={{ width: `${progressPercent}%` }} />
              </div>
            </div>
            ...
            <div className="mt-4 flex gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-3 sm:p-4 text-xs sm:text-sm text-(--text-primary)/70">
              ...
              <p>Vui lòng không đóng trang trong khi chờ xác nhận thanh toán.</p>
            </div>
          </section>
          ...
          <aside className="space-y-4 lg:col-span-5">
            ...
            <section className="rounded-3xl border border-(--text-primary)/10 bg-(--surface-color)/80 p-4 sm:p-5 shadow-[0_24px_80px_rgba(0,0,0,0.10)]">
              ...
              <div className="mt-4 space-y-3 sm:mt-5">
                <DetailRow icon={CreditCard} label="Số tiền" value={formatCurrency(amount)} valueClassName="text-lg sm:text-xl text-(--primary-color)" />
                <DetailRow icon={Building2} label="Ngân hàng" value={bankCode} />
                <DetailRow
                  icon={CreditCard} label="Số tài khoản" value={accountNumber}
                  action={<CopyButton value={accountNumber} copied={copiedField === accountNumber} onCopy={handleCopy} />}
                />
                <DetailRow icon={CheckCircle2} label="Chủ tài khoản" value={accountName} />
              </div>

              {/* Transfer content */}
              <div className="mt-4 rounded-2xl border border-(--primary-color)/25 bg-(--primary-color)/10 p-3 sm:p-4">
                <p className="text-xs sm:text-sm text-(--text-primary)/60">Nội dung chuyển khoản</p>
                <div className="flex flex-col gap-3 mt-3 sm:flex-row sm:items-center">
                  ...
                  <CopyButton value={transferContent} copied={copiedField === transferContent} onCopy={handleCopy} />
                </div>
              </div>
              ...
              <div className="mt-4 flex gap-3 rounded-2xl border border-amber-500/25 bg-amber-500/10 p-3 sm:p-4 text-xs sm:text-sm text-(--text-primary)/75">
                ...
                <p>Vui lòng không thay đổi nội dung chuyển khoản. Nếu thay đổi, hệ thống có thể không tự động xác nhận thanh toán.</p>
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-(--text-primary)/10 bg-(--surface-color)/80 p-4 sm:p-5 shadow-[0_24px_80px_rgba(0,0,0,0.10)]">
              ...
              <div className="grid gap-3 mt-4 text-xs sm:text-sm">
                {[
                  { label: 'Sự kiện', value: order.event?.title ?? 'EventHub', className: 'max-w-[180px] sm:max-w-56 text-right font-semibold text-(--text-primary)' },
                  { label: 'Địa điểm', value: order.event?.location ?? 'N/A', className: 'max-w-[180px] sm:max-w-56 text-right font-semibold text-(--text-primary)' },
                  {
                    label: 'Thời gian',
                    value: order.event?.startDate
                      ? new Date(order.event.startDate).toLocaleString('vi-VN', { dateStyle: 'medium', timeStyle: 'short' })
                      : 'N/A',
                    className: 'text-right font-semibold text-(--text-primary)',
                  },
                  { label: 'Số lượng vé', value: order.ticketCount ?? order.orderSeats?.length ?? 'N/A', className: 'font-semibold text-(--text-primary)' },
                  { label: 'Tổng tiền', value: `${Number(order.totalAmount ?? 0).toLocaleString('vi-VN')}đ`, className: 'font-semibold text-(--primary-color)' },
                ].map(({ label, value, className }) => (
                  <div key={label} className="flex justify-between gap-4">
                    <span className="shrink-0 text-(--text-primary)/55">{label}</span>
                    <span className={className}>{value}</span>
                  </div>
                ))}

                <div className="flex justify-between gap-4">
                  <span className="shrink-0 text-(--text-primary)/55">Trạng thái</span>
                  <span className="rounded-full bg-yellow-500/10 px-2.5 py-1 text-xs font-semibold text-yellow-500">
                    {order.status === 'PENDING' ? 'Đang chờ thanh toán' : order.status}
                  </span>
                </div>

                <div className="flex justify-between gap-4">
                  <span className="shrink-0 text-(--text-primary)/55">Mã đơn hàng</span>
                  <span className="font-mono text-xs font-semibold text-(--text-primary) break-all text-right">{displayOrderId}</span>
                </div>
              </div>
            </section>
          </aside>
