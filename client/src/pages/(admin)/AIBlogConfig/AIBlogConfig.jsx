...
function AIBlogConfig() {
    ...
    const fetchBlogIdeas = async () => {
        try {
            const res = await aiBlogConfigService.listBlogAi({
                page: 1,
                size: 10,
            });

            setBlogIdeas(res.items || []);
        } catch (err) {
            console.error(err);
        }
    };
    const fetchConfig = async () => {
        try {
            const res = await aiBlogConfigService.getAIConfig();
            ...
            setThumbnailModel(res.thumbnailModel || '');
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const handleSave = async () => {
        try {
            await aiBlogConfigService.updateAIConfig(config.id, {
                ideaModel,
                ideaPrompt,
                blogModel,
                blogPrompt,
    ...
    return (
        <div className="space-y-2">
            ...
            <AIModelCard
                ...
                setSelectedAI={setThumbnailModel}
                content="ảnh"
            />
            ...
            <NumberLimitForm onSubmit={handleGenerateIdeas} />

            <StatusTable data={blogIdeas} />
        </div>
    );
}
